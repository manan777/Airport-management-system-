...
session_start();
    $city = $_GET['id'];
   $sql ="SELECT ap_name,state,country,cname
   from airport a
   where a.cname = '$city'";
   ...
   if(mysqli_num_rows($result) >0)
    {
        ...
        while($row = $result->fetch_assoc()){
            echo"<tr><td><a href='contains.php?id=$row[ap_name]'>$row[ap_name]</a></td><td>".$row["state"]."</td><td>".$row["country"]."</td><td>".$row["cname"]."</td></tr>";
